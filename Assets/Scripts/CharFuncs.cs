...
public class CharFuncs : MonoBehaviour {
	...
	public string mynextline = "This is my next line.";
...
//			for(CharFuncs a : GlobalObjs.listOfChars) {
	...
	void OnGUI() {
		...
		if(speaking) {
			// don't need to show the text anymore, so remove this line
			//GUI.Label (new Rect(150, 630, 1000, 30), new GUIContent(saywhat, myicon), mystyle);	
			
			//Debug.Log (saywhat);
			//Debug.Log (Camera.main.name + " & " + thisChar.name + " position=" + Camera.main.WorldToScreenPoint(thisChar.transform.position));
			Vector3 ptheight = Camera.main.WorldToScreenPoint(thisChar.transform.position + new Vector3(0f, 6.5f, 0f));
			Vector3 ptheightabove = Camera.main.WorldToScreenPoint(thisChar.transform.position + new Vector3(0f, 7.5f, 0f));
			Vector3 pt = Camera.main.WorldToScreenPoint(thisChar.transform.position);
			//Debug.Log ("ph="+ptheight+", pha="+ptheightabove+", p="+pt);
			float heightdiff = ptheightabove.y - ptheight.y;
			float width = (2.5f) * heightdiff;
			float bubbleheight = 1.15f * width;
			float bubblewidth = width;
			float startbubbley = Screen.height - (ptheightabove.y +(bubbleheight/2));//ptheight.y;//Screen.height - ptheightabove.y -25f;
			float startbubblex = ptheight.x - (bubblewidth/2);//ptheight.x - (.4f*width);
